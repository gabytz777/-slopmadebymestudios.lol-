...
(function (root, factory) {
  ...
  const ACTIONS = {
    ...
    give: {
      label: "give item",
      needsPlayer: true,
      imports: ["net.vibmc.item.ItemStack", "net.vibmc.item.ItemType"],
      code: (a, ctx) => [
        `${ctx.player}.addItem(new ItemStack(ItemType.${(a.value || "diamond").toUpperCase()}, ${clampInt(a.amount, 1, 64, 1)}));`,
      ],
    },
    teleport: {
      label: "teleport",
    ...
    heal: {
      ...
      code: (a, ctx) => [
        ...
        // The bar only moves if the client is told about it.
        `VibMC.getInstance().getPlayerManager().sendHealth(${ctx.player});`,
      ],
    },
    time: {
      label: "set time",
    ...
    kick: {
      ...
      needsPlayer: true,
      imports: [],
      code: (a, ctx) => [`${ctx.player}.kick(${jq(a.value || "Bye!")});`],
    },
    log: {
